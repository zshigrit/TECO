...
!       USE IFPORT !! Was: USE DFPORT
      USE DFPORT      
      implicit none
      integer, parameter :: No_site=1!2            ! the sites to run
      integer, parameter :: ipara=21             ! governing parameters
      integer, parameter :: ioutput=12           !
!       integer, parameter :: iiterms=16         ! 16 for Oak Ridge input
      integer, parameter :: iiterms=8            ! 9 for Duke forest FACE
!       integer, parameter :: ilines=180560      ! the maxmum records of Oak Ridge FACE, 1999~2007
...
!     for loops
      ...
      integer year,yr,days,i,j,k,m,n,yrs_eq,hoy,iyr
...
!    scenario
      ...
      integer ScTair
      character(len=50) climfile,NEE_file,modelsite,out_d,out_h
...
!     0.7 gN/yr/m2, 13.4 kg N ha-1 yr-1, 2000, Dentener et al. 2006, GBC, Duke FACE
      N_deposit=0!2.34/8760. !(gN/h/m2, ) zheng
...
! N_up_yr,N_fix_yr,N_dep_yr,N_leach_yr,N_vol_yr
          write(63,'(a250)')"yr,year,gpp_yr,Rnitr,NPP_yr,Rh_yr,NEE_yr,
     ...
     &Q_L,Q_W,Q_R,Q_fine,Q_coarse,Q_M,Q_S,Q_P,
     &N_leaf,N_wood,N_root,N_fine,
...
!          QC=(/100.,800.,100.,39.,100.,122.,834.,12./)
          QC=(/100.,10.,100.,39.,100.,122.,834.,12./)! zheng
!          QC=(/300.,4300.,300.,119.,300.,322.,3834.,312./) zheng
...
!            using ambient data to run equilibiurm, elevated only for the last cycle
             if(isite==2.and.yr>yrs_eq)input_data(5,:)=input_elev(5,:)  ! turned on by Weng 2012-03-13
             iyr=iyr+1
             if(iyr>yr_data)iyr=1

             if(yr>yrs_eq)then
                 input_data(1,:)=input_data0(1,:)+ScTair
                 input_data(2,:)=input_data0(2,:)+ScTair
             endif
...
!              endif
              ...
              StemSap=bmleaf!AMIN1(Stemmax,SapS*bmStem) zheng
              RootSap=bmroot!AMIN1(Rootmax,SapR*bmRoot) zheng
              NSCmin=5. 
...
!!       for Duke Forest
                  ...
                  co2ca=input_data(5,m)*1.0E-6 ! CO2 concentration,ppm-->1.0E-6
!                  if(isite==2.and.yr>yrs_eq)then
!                      co2ca=(input_data(5,m)+200.)*1.0E-6
!                  endif

                  rain=input_data(6,m)    ! rain fal per hour
                  wind=ABS(input_data(7,m))     ! wind speed m s-1
                  PAR=input_data(8,m)             ! Unit ? umol/s/m-2
                  radsol=input_data(8,m)        ! unit ? PAR actually
!                  Rnet=input_data(9,m)
!!       endof Duke Forest

!                 Ajust some unreasonable values
                  RH=AMAX1(0.01,AMIN1(99.99,RH))
                  eairP=esat(Tair)-Dair
...
!                 for daily mean conditions
                  ...
                  PAR_D=PAR_D+radsol/dtimes                 ! umol m-2 s-1
...
!                 output hourly
                  ...
                  if((yr.gt.yrs_eq).and.(yr.le.(yrs_eq+yr_data)))then
                      write(61,161)year,hoy,co2ca*1.0E6,rain,PAR,LWh,
     &                  Tair,Tsoil,VPDh,wscontent,NEP,GPP,NPP,CEXh,
     &                  CVOCh,Recoh,Rauto,RaLeaf,RaStem,RaRoot,
     &                  Rgrowth,Rhetero,Rsoil,ETh,Th,Eh,INTh,ROh,
     &                  DRAINh,LEh,SHh              
                  endif
!                 sums of a day
...
!     &NDW,NSOIL,NPOOLM,NPOOLO,NFIX,NLITIN,NWLIN,NRLIN,NUP,NGMIN,
!     &NMIN,NVOL,NLEACH,NGL,NGW,NGCR,NGR"
!              write(*,*)LAI
              if((yr.gt.yrs_eq).and.(yr.le.(yrs_eq+yr_data)))then
!                             YEAR ,DOY,CO2,       PPT,    PAR, AT,ST,
...
162           format(I6,",",f8.0,",",120(f12.4,","))
            enddo                         ! end of a year
            storage=accumulation
            stor_use=Storage/times_storage_use
            write(*,*)year,LAI,gpp_yr,NPP_yr,NEE_yr
...
!     RD=BM*Rd*Q10**((T-25)/10) (Sun et al. 2005. Acta Ecologica Sinica)
      ...
      RmLeaf=Rl0*SNRauto*bmleaf*0.45*SLA*0.1
...
!      c2=0.5*250e3*SLA*0.00021*ht*2.
      !write(*,*)LAI
      GrowthL=MAX(0.0,MIN(GrowthP*0.2,0.001*(LAIMAX-LAI)/SLA))    ! 1./(1.+c1+c2)
      GrowthR=MIN(GrowthP*0.4,MAX(0.0,7.5/Sw*bmL-bmR))  ! *c1/(1.+c1+c2)
